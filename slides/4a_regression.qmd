...
. . . 
  ...
  - **Solution**: no simpler statement than the problem above [(except $\left(\mathbb{X}'\mathbb{X}\right)^{-1} \, \mathbb{X}'\mathbf{Y}$)]{.gray}
...
The $R^2$ measures how predictive the predictors are [(in-sample)]{.gray}:
...
combinations <- expand_grid(treat_deshawn = c(0,1),
                            leg_R = c(0,1))
mod_rigid <- lm(reply_atall ~ treat_deshawn + leg_R, data = bb)
mod_flex <- lm(reply_atall ~ treat_deshawn*leg_R, data = bb) 
...
- because we want to predict the outcome at specific values of the predictors: e.g. $\hat{Y}_i = \hat{\beta}_0 + 2\hat{\beta}_1 + 3\hat{\beta}_2$
...
ci_data <- data.frame(
  ...
  estimate = c(coef(intx_reg)["treat_deshawn"], # effect for Dems
               sum(coef(intx_reg)[c("treat_deshawn", "treat_deshawn:leg_R")])), # effect for R
...
mod <- lm(y ~ x, data = dat)
results <- expand_grid(k = c(1, 2, 3, 4, 5, 6, 7),
            rep = 1:200,
            n = c(15, 50, 100, 500)) |> 

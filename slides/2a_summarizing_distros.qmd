...
format: 
  revealjs:
    smaller: false
    slide-number: c/t
    css: ["uchicago_pol_meth.css", "default"]
    theme: simple
    echo: false
...
Let $X$ and $Y$ be RVs. Then $\forall a, b, c \in \mathbb{R}$, $\E[aX + bY + c] = a\E[X] + b\E[Y] + c$
...
$$\begin{aligned}
\V[X] &= \E[(x - \E[X])^2] \\ &= \color{red}{p^2}\color{green}{(1-p)} + \color{orange}{(1 - 2p + p^2)}\color{blue}{p} \\ &= p^2 - p^3 + p - 2p^2 + p^3 \\ &= p(1 - p)\end{aligned}$$
...
&= \E\left[X^2 - \color{blue}{2\E[X]} X + \E[X]^2\right] \\
&= \E[X^2] - \E\left[\color{blue}{2\E[X]} X\right] + \E\left[\E[X]^2\right] \\
&= \E[X^2] - \color{blue}{2\E[X]} \E[X] + \E[X]^2 \\ 
&= \E[X^2] - 2 \E[X]^2 + \E[X]^2 \\ 
&= \E[X^2] - \E[X]^2
\end{align}$$
...
&= \E\left[(X + c - \E[X] - \E[c])^2\right] \\
&= \E\left[(X + c - \E[X] - c)^2\right] \\
...
## Revisiting justification for $\E[X]$  {.smaller}
...
&= \E[X^2] - 2c\E[X] + c^2 \\
&= \E[X^2] - \color{red}{\E[X]^2} + \color{green}{\E[X]^2} - 2c\E[X] + c^2 \\
&= \left(\E[X^2] - \color{red}{\E[X]^2}\right) + \left(\color{green}{\E[X]^2} - 2c\E[X] + c^2\right) \\

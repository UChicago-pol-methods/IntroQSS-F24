...
\V[X] &= \E\left[  \color{blue}{(X - \E[X])}\color{blue}{(X - \E[X])} \right]\end{align}
...
- if $\E[X] = \E[Y] = 0$ (e.g. if recentered), both are $\E[XY]$
...
[i.e. (1,1) and (3,3) equally likely]{.smaller}
...
[i.e. (1,3) and (3,1) equally likely]{.smaller}
...
exey_box <- tibble(x = c(0, 2, 2, 0, 0),
                   y = c(0, 0, 2, 2, 0))
p1 + 
  geom_vline(xintercept = 2, lty = 2) + 
  geom_hline(yintercept = 2, lty = 2) + 
  geom_polygon(data = exey_box, fill = "gray", alpha = .5, show.legend = F)
...
[$\E[X]\E[Y]$ is the area below and to the left of the dashed lines]{.smaller}
...
[$\E[XY]$ is the average of the two areas (given equal probability)]{.smaller}
...
$$(\alpha, \beta) = \underset{(a,b) \in \mathbb{R}^2}{\arg\min} \, \E\,[\left(Y - (a + bX)\right)^2]$$
...
$$(\alpha, \beta) = \underset{(a,b) \in \mathbb{R}^2}{\arg\min} \, \E[\left(\E[Y|X] - (a + bX)\right)^2]$$

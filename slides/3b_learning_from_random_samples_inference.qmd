...
- centered on $\E[X]$, i.e. **unbiased**: $\E[\overline{X}] = \E[X]$
- variance is $\V[X]/n$, which we can approximate with $\hat{V}[X]/n$
...
Suppose we knew $\E[X]$ and $\V[X]$. 

. . . 

Then we know the *asymptotic* sampling distribution of $\overline{X}$, i.e. $N\left(\E[X], \frac{\V[X]}{n}\right)$. 

. . . 

We could then compute, e.g. the probability of observing a sample mean below 2 if $\E[X] = 2.104$, $\V[X] = 4.63$, and $n = 198$. 
...
Similar if we only knew $\E[X]$ but not $\V[X]$.

## The researcher's perspective

But we don't know $\E[X]$ (that's why we're doing research). 

. . . 

We have an estimate $\overline{X}$, and we can estimate $\V[X]$ from the sample. What more can we say about $\E[X]$?  

. . . 

Reporting $\hat{\sigma}[\overline{X}] = \sqrt{\frac{\hat{\V}[X]}{n}}$, the **standard error**, is a start.

. . . 

Approaches we will take: 

- what is an interval centered on $\overline{X}$ that will contain $\E[X]$ in at least 95% of samples? (**confidence interval**)
- suppose $\E[X] = c$. what is the probability of observing $\overline{X}$ at least as far from $c$ as what we observed? (**p-value**) 
 
## The researcher's perspective (2) {.smaller}

Generally, these tools of statistical inference require 

- having a plug-in estimator for $\theta$, i.e. $\hat{\theta}$ 
- having a plug-in estimator for $\V[\hat{\theta}]$, i.e. $\hat{\V}[\hat{\theta}]$
...
That's quite a lot! [(We also know approximate shape of any plug-in estimator under "mild regularity conditions"; can get location and spread.)]{.gray}
--> 
...
```{r}
#| echo: true
# -1.96 is the value of a standard normal 
# i.e. N(0, 1)
# that is above 2.5% of mass 
pnorm(q = -1.96, mean = 0, sd = 1)

# what if not standardized? 
me <- -.345
st <- 1.23 
pnorm(q = me - 1.96*st, mean = me, sd = st)

# so 95% is between these
pnorm(q = 1.96) - pnorm(q = -1.96) # std normal
pnorm(me + 1.96*st, mean = me, sd = st) - 
  pnorm(me - 1.96*st, mean = me, sd = st)
```
...
What about in a particular case? Can I say, "The probability that $\theta$ is in this CI is .95"?

. . .

- Strict frequentist says "**NO.** $\theta$ is not random; it is in this CI or it isn't. Probability is not for your beliefs."
- I (not a strict frequentist) say, "It's OK to describe beliefs with probability. 95% of my CIs contain $\theta$, so $\text{Pr}(\theta \in \text{this CI}) \approx .95$." --><!-- assuming you don't know anything else --> 
...
$\theta_0$ is called the *null hypothesis*. "Under the null" means "assuming $\theta = \theta_0$".
...
<!-- *Asymptotically, and if $\text{V}[\hat{\theta}]$ under the null equals $\E\left[\hat{\V}[\theta]\right]$  --> 
...
Two-tailed p-value: 
$\text{Pr}\left[\lvert\hat{\theta} - \theta_0\rvert \geq \lvert\hat{\theta}^* - \theta_0\rvert \right]$ under the null.


<!-- $\text{Pr}\left[\lvert\hat{\theta} - \theta_0\rvert \geq \lvert\hat{\theta}^* - \theta_0\rvert \bigm| \theta = \theta_0\right]$--> 
...
Under the null, asymptotically $t \sim N(0, 1)$.

. . . 

Like $z$-score ($\frac{X - \overline{X}}{\sigma[X]}$), $t$-statistic is a standardization/normalization. 
...
In principle, both interesting. One-tailed especially relevant if the null hypothesis is really that $\theta \geq \theta_0$ or $\theta \leq \theta_0$

. . . 

In practice, we only use one-tailed tests when the test is *pre-registered*. Otherwise two-tailed to be conservative. Why? <!-- given the way $p$-values are used in testing. --> 
...
$\V[\hat{\theta}]$ describes the variance of $\hat{\theta}$ across samples [of size $n$]{.gray}.

**Problem:** We have only one sample [of size $n$]{.gray}.
...
sto <- rep(NA, times = m)
for(i in 1:m){
  resamp <- sample(samp, size = length(samp), replace = T)
  sto[i] <- mean(resamp)
}
```
...
```{r}
#| echo: true
# these are our simulated sample means 
head(sto) 
# our bootstrap estimate of the variance of the sample mean:
var(sto) 
# our bootstrap estimate of the standard error (of the sample mean):
sd(sto) 
```


## Does this work? (And if so why?) {.smaller}

The bootstrap is a **plug-in estimator**!

- Our estimand is $\V[\hat{\theta}]$, the sampling variance
...
- randomly reshuffle the treatment [-- e.g. instead of `1, 1, 0, 1, 0, 0` (actual treatments ), use `1, 0, 0, 1, 1, 0`]{.gray}
...
## Randomization inference example {.smaller}
...
load("./../data/ggl.RData")
ggl |> 
  filter(treatment %in% c("Control", "CivicDuty")) |> 
  group_by(treatment, hh_id, hh_size) |> 
  summarize(voted = mean(voted)) |> 
  ungroup() -> d2
```

Green, Gerber, and Larimer assign households to receive GOTV fliers or not.

```{r}
#| echo: false
d2 |> 
  slice_sample(n = nrow(d2)) |> 
  select(-hh_id) -> tmp
...
Turnout rate by type of household:

```{r}
#| echo: true
(treat_mean <- mean(d2$voted[d2$treatment == "CivicDuty"]))
(control_mean <- mean(d2$voted[d2$treatment == "Control"]))
```

```{r}
#| echo: true
(true_diff <- treat_mean - control_mean)
```


## Randomization inference example (2) {.smaller}


What is the probability of getting a turnout difference as big as `r round(true_diff, 3)` if the treatment has no effect on any household? 

. . . 

```{r}
#| echo: true
m <- 1000; sto <- 1:m; d3 <- d2
for(i in 1:m){
  d3$voted <- sample(d3$voted)
  sto[i] <- mean(d3$voted[d3$treatment == "CivicDuty"]) - 
mean(d3$voted[d3$treatment == "Control"])
}
```
...
```{r}
hist(sto, xlim = c(-true_diff, true_diff))
abline(v = true_diff)
```
...
If we are going to report measures of uncertainty, we need to specify what is uncertain.

. . . 

Our focus was on *sampling* from a population -- the most straightforward. 

. . . 

In randomization inference, we focused on *treatment assignment* as another source of uncertainty.

. . . 

In most social science research, source of uncertainty not specified, but could be 

- sampling from a *super-population*: there could have been other OECDs out there!
- model-based uncertainty -- treat the error term as random, e.g. due to measurement error or other unknown factors

<!-- TODO: bootstrap picture: sampling from the sample instead of the population --> 

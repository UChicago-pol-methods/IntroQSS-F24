...
<!-- TODO: explain the z-scores in the CLT part. --> 
...
<!-- variance across the sample means. don't give away too much because it's in the homework.--> 
...
<!-- Recall, $\overline{X}_{(n)}$ is the sample mean based on a sample of size $n$. -->
...
datt |> 
  ...
  theme_bw() + 
  labs(x = "Sample size (n)", y = "Cumulative mean", title = "1 run (starting at n=20)") + 
...
## $\E[\overline{X}] = \E[X]$ illustration

```{r}
datt |> 
  ungroup() |> 
  group_by(j) |> 
  mutate(ebarx = mean(cm)) |> 
  ggplot(aes(x = j, y = ebarx)) + 
  geom_line() + 
  theme_bw() +
  geom_hline(yintercept = 2/3, lty = 2) + 
  expand_limits(y = c(.64, .7)) +
  labs(x = "Sample size", y = "Mean of the 500 sample means")
```

## $\V[\overline{X}] = \V[X]/n$ illustration
...
datt |> 
  ungroup() |> 
  group_by(j) |> 
  mutate(vbarx_Empirical = var(cm),
         vbarx_Theoretical = (2/3)*(1 - 2/3)/j) |>
  pivot_longer(cols = starts_with("vbarx_"), names_prefix = "vbarx_", names_to = "type", values_to = "vbarx") -> for_vbar_plot
```


```{r}
for_vbar_plot |> 
  ggplot(aes(x = j, y = vbarx, lty = type)) + 
  geom_line() + 
  scale_y_log10() +
  theme_bw() +
  labs(x = "Sample size", y = "Variance of the 500 sample means\n(log scale)",
       lty = "")
```
...
[Could also do $\overline{(X - \overline{X})^2}$.]{.gray}
...
So $\E\left[\hat{V}_{\text{plug-in}}[X]\right] = 0 \times 1/2 + 1/4 \times 1/2 = 1/8 < \V[X] = 1/4$. 
...
<!-- so we get the target  minus the degree to which the sample mean is bouncing around --> 
...
- $\V[\overline{X}]$: the (sampling) variance of the sample mean [($= \frac{\V[X]}{n}$)]{.gray}
- $\hat{\V}_{\text{plug-in}}[X]$: the plug-in sample variance [($\overline{X^2} - \overline{X}^2$, a biased estimator of $\V[X]$)]{.gray}
- $\hat{\V}[X]$: the sample variance  [($\frac{n}{n-1}\hat{\text{V}}_{\text{plug-in}}[X]$, an unbiased estimator of $\V[X]$)]{.gray}
...
- we want to report the variance of the sample mean, $\V[\overline{X}] = \frac{\V[X]}{n}$, but we don't know $\V[X]$ 
- we instead report $\hat{\V}[X]/n$, an *estimator* of the variance of the sample mean 
...
## Estimand/estimator wrap-up{.smaller}

| Estimand   | Estimator  | Biased? |   
|:--------------------:|:------------:|:------------:|
| Pop. mean, $\E[X]$   |   $\overline{X}$   |   $\E[\overline{X}] = \E[X]$  |  
| Pop. variance, $\V[X]$   |  $\hat{\V}_{\text{plug-in}}[X]$  |  $\E\left[\hat{\V}_{\text{plug-in}}[X]\right] = \frac{n-1}{n} \V[X]$     |
| Pop. variance, $\V[X]$   |  $\hat{\V}[X]$  |   $\E[\hat{\V}[X]] = \V[X]$ |
| (Sampling) var. of sample mean, $\V[\overline{X}]$   |  $\frac{\hat{\V}[X]}{n}$  |   $\E[\hat{\V}[\overline{X}]] = \V[\overline{X}]$ |
...
- $\overline{X}_{(n)} \overset{p}{\to} \E[X]$ [(WLLN)]{.gray}
...
```{r, echo = F}
sample_and_plot <- function(n, point_size = 1, point_alpha = 1, line_alpha = 1, line_width = .5, seed = 123, m = 10000){
  set.seed(seed)
  x <- c(0, 1)
  samp_means <- replicate(m, mean(sample(x, size = n, replace = T)))

  tibble(sm = round(samp_means, 7)) |> 
    count(sm) -> counts 
  ...
  for_plot %>% 
    ggplot(aes(x = sm, y = n)) + 
    geom_point(data = filter(for_plot, type == "tip"), alpha = point_alpha, size = point_size) +
    geom_line(aes(group = factor(sm)), alpha = line_alpha, lwd = line_width) +
    labs(x = "Sample mean", y = "Frequency in 10,000 draws", subtitle = str_c("Sample size: ", n)) + 
    theme_bw() 
}
sample_and_plot(n = 2)
```
...
```{r, echo = F}
sample_and_plot(n = 1000, point_size = .5, point_alpha = .5, line_alpha = .3, line_width = .1) + 
...
```{r, echo = F}
sample_and_plot(n = 1000) + 
  coord_cartesian(xlim = c(.43, .57))
```


## Case 4a: $n = 1000$, $m = 1,000,000$

```{r, echo = F}
sample_and_plot(n = 1000, m = 1000000) + 
  coord_cartesian(xlim = c(.43, .57))
```
...
i.e. the normal distribution with mean $\mu$ and variance $\frac{\sigma^2}{n}$.
...
- $\E[\overline{X}] = \E[X] = \mu$
- $\V[\overline{X}] = \frac{\V[X]}{n} = \frac{\sigma^2}{n}$
...
## Central limit theorem (two forms) {.smaller}
...
\begin{align}
\frac{\sqrt{n} \left(\overline{X} - \mu\right)}{\sigma} &\overset{d}{\to} N(0, 1)  \tag{Version 1} \\
\overline{X} - \mu &\overset{d}{\to}  \frac{\sigma}{\sqrt{n}} N(0, 1) \\
\overline{X} - \mu &\overset{d}{\to}   N\left(0, \frac{\sigma^2}{n} \right) \\
\overline{X} &\overset{d}{\to}   N\left(\mu, \frac{\sigma^2}{n} \right)  \tag{Version 2}
\end{align}
...
## When the CLT does not apply {.smaller}
...
- $\E[X] = \infty$, e.g. [the St Petersburg paradox](https://en.wikipedia.org/wiki/St._Petersburg_paradox)
- $\V[X] = 0$
...
```{r, echo = F, cache = T, fig.height = 6}
the_mode <- function(x){
  names(sort(table(x), decreasing = T))[1] |> as.numeric()
}

expand_grid(n = c(3, 10, 20, 100, 1000, 2000),
            rep = 1:10000) |> 
  mutate(stat = map_dbl(n, ~the_mode(sample(x = c(0,1), size = .x, replace = T, prob = c(.5, .5))))) -> dat

plot_function_mode <- function(dat, title = "X is Normal"){
  
  dat |> 
    mutate(facet_labs = fct_reorder(str_c("n = ", n), n)) |> 
    ggplot(aes(x = stat)) +
    geom_histogram() + 
    facet_wrap(vars(facet_labs)) + 
    labs(x = "Sample mode", y = "Density", title = title)
}

plot_function_mode(dat, title = "Sample mode of X, which is Bernoulli w. p = .5")
```
...
```{r}
k <- 10
expand_grid(n = c(3, 11, 21, 101, 1001, 2001),
            rep = 1:10000) |> 
  mutate(stat = map_dbl(n, ~median(sample(x = 1:k, size = .x, replace = T, prob = rep(1/k, k))))) -> dat

dat |> 
  mutate(facet_labs = fct_reorder(str_c("n = ", n), n)) |> 
  ggplot(aes(x = stat)) +
  geom_histogram(breaks = seq(.75, 10.25, by = .5)) + 
  facet_wrap(vars(facet_labs)) + 
  labs(x = "Sample median", y = "Density", title = "Sample median of X, which is uniform over integers 1-10")
```
